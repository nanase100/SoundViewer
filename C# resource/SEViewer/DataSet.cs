...
namespace SEViewer
{
	...
	public class readJsonType1  {
		...
		public List<string>	タブ毎命令文					{ get; set; }  = new List<string>();
	};
	...
	class DataSetManager
	{
		...
		public string copyStrBGM { set; get; } = "";
		public string copyStrSE { set; get; } = "";

		public List<string> copyStrByTab { set; get; }
		...
		public bool settingLoad(string settingFilePath )
		{
			...
			copyStrBGM = jsonData.BGM停止命令文;
			copyStrSE = jsonData.SE停止命令文;

			foreach( var tmp in jsonData.タブ毎命令文) copyStrByTab.Add( tmp );
			...
			for (int i = 0; i < MAX_CATEGORY; i++)
			{
				...
				//
				//favリストの先頭に "_" がある場合、つまり_optionと同じ形式を狙っている場合
				if( i == MAX_CATEGORY-1)
				{
					if( workPath.IndexOf("_") == 0 )
					{
						workPath = workPath.Substring( 1 );
					}
					else
					{
						//そうでなければ逆に先頭に "_" を追加する
						workPath = "_" + workPath;
					}
				...
				//csvを読み込
				System.IO.StreamReader streamData = new System.IO.StreamReader(workPath, System.Text.Encoding.Default);
		...
		public bool settingSave(string settingFilePath)
		{
			...
			jsonData.BGM停止命令文 = copyStrBGM;
			jsonData.SE停止命令文 = copyStrSE;

			jsonData.タブ毎命令文 = copyStrByTab;
		...
		{
			//ここから設定の書き込み
			string saveFavFilePath = this.txtPath[MAX_CATEGORY-1];

			//favリストの先頭に "_" がある場合、つまり_optionと同じ形式を狙っている場合
			if( saveFavFilePath.IndexOf("_") == 0 )
			{
				saveFavFilePath = saveFavFilePath.Substring( 1 );
			}
			else
			{
				//そうでなければ逆に先頭に "_" を追加する
				saveFavFilePath = "_" + saveFavFilePath;
			}

			System.IO.StreamWriter streamData = new System.IO.StreamWriter( saveFavFilePath, false, System.Text.Encoding.Default);
